classdef remApi
    ...
    properties (Constant)
        ...
        simx_modo_bloqueando            =65536;
        simx_opmode_oneshot_wait        =65536;
        simx_modo_espera_unavez        =65536;
        simx_opmode_continuous          =131072;
        simx_modo_continuo         =131072;
        simx_opmode_streaming           =131072;
        simx_modo_retransmision          =131072;
    ...
    methods
        ...
        function obj = remApi(libname,hfile)
            ...
            disp('Nota: Asegurese de siempre usar la la libreria remoteApi correspondiente');
            disp('(i.e. 32bit Matlab no funcionará con la remoteApi de 64bit,y viceversa)');
        ...
        function [rtn] = simxComenzar(obj,doNotReconnectOnceDisconnected,timeOutInMs,commThreadCycleInMs)
            server_intval = int8(['127.0.0.1',0]);
            server_ptr = libpointer('int8Ptr',server_intval);
            rtn = calllib(obj.libName,'simxStart',server_ptr,19999,uint8(true),uint8(doNotReconnectOnceDisconnected),timeOutInMs,commThreadCycleInMs);
        end

        function simxFinalizar(obj,clientID)
        ...
        function [rtn position] = simxLeerPosicionMotor(obj,clientID,handle,option)
        ...
        function [rtn handle] = simxObtenerObjeto(obj,clientID,name,operationmode)
        ...
        function [rtn eulerAngles] = simxLeerOrientacion(obj,clientID,objectHandle,relativeToObjectHandle,operationMode)
            objectHandle_ = int32(objectHandle);
        ...
        function [rtn position] = simxLeerPosicion(obj,clientID,objectHandle,relativeToObjectHandle,operationMode)
            objectHandle_ = int32(objectHandle);
            relativeToObjectHandle_ = int32(relativeToObjectHandle);
            operationMode_ = int32(operationMode);
...
%       end
        ...
        function [rtn resolution_ image] = simxObtenerImagenSensorVision(obj,clientID,handle,options,operationmode)
            resolution = [0 0];
            resolution_ = libpointer('int32Ptr',int32(resolution));
        ...
        %simxObtenerImagendelSensordeVision
        function [rtn resolution_ image] = simxObtenerImagenSensorVision2(obj,clientID,handle,options,operationmode)
            resolution = [0 0];
            resolution_ = libpointer('int32Ptr',int32(resolution));
        ...
        function [rtn force]= simxLeerFuerzaMotor(obj,clientID,jointHandle,operationMode)
            jointHandle_ = int32(jointHandle);
            operationMode_ = int32(operationMode);
            force = libpointer('single',single(0));
        ...
        function rtn = simxPausarSimulacion(obj,clientID,operationMode)
            operationMode_ = int32(operationMode);
        ...
        function [rtn minimumDistance]= simxLeerDistancia(obj,clientID,distanceObjectHandle,operationMode)
      ...
      if ~isempty(auxValuesCount_),
        ...
        function [rtn ]= simxFijarOrientacion(obj,clientID,objectHandle,relativeToObjectHandle,eulerAngles,operationMode)
            objectHandle_ = int32(objectHandle);
            relativeToObjectHandle_ = int32(relativeToObjectHandle);
        ...
        function rtn = simxFijarPosicion(obj,clientID,handle,rel_pos,position,option)
            if (numel(position) < 3)
...
%           end
        ...
        function rtn = simxComenzarSimulacion(obj,clientID,operationMode)
            operationMode_ = int32(operationMode);

            rtn = calllib(obj.libName,'simxStartSimulation',clientID,operationMode_);
        end

        function rtn = simxPararSimulacion(obj,clientID,operationMode)
            operationMode_ = int32(operationMode);
        ...
        function [rtn linearVelocity angularVelocity]= simxLeerVelocidad(obj,clientID,objectHandle,operationMode)
            objectHandle_ = int32(objectHandle);
        ...
        function rtn = simxFijarPosicionMotor(obj,clientID,handle,position,option)
            handle_ = int32(handle);
        ...
        function [rtn ]= simxFijarVelocidadObjetivoMotor(obj,clientID,objectHandle,targetVelocity,operationMode)
            objectHandle_ = int32(objectHandle);
        ...
        function [rtn ]= simxFijarPosicionObjetivoMotor(obj,clientID,objectHandle,targetPosition,operationMode)
            objectHandle_ = int32(objectHandle);
        ...
        function [rtn detectionState detectedPoint detectedObjectHandle detectedSurfaceNormalVector]= simxLeerSensorProximidad(obj,clientID,sensorHandle,operationMode)
